            ...
            <div class="col-md-4">
                ...
                <form:form method="POST" action="/station/add" modelAttribute="station">
                    ...
                    <div class="form-group">
                        ...
                        <form:input path="stationTitle" class="form-control"/>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            <form:label path="lat">Lat</form:label>
                            <form:input path="lat" id="latInput" class="form-control"/>
                        </div>
                        <div class="col-md-6">
                            <form:label path="lng">Lng</form:label>
                            <form:input path="lng" id="lngInput" class="form-control"/>
                        </div>
                    </div>
                    <c:forEach var="st" items="${stations}">
                        <div class="checkbox">
                            <label>
                                <form:checkbox path="arriveStationIds" value="${st.stationId}" data-lat="${st.lat}" data-lng="${st.lng}"></form:checkbox>
                                    ${st.stationTitle}
                            </label>
                        </div>
                    </c:forEach>
...
function initMap() {
    ...
    function drawLines() {
        $('[name="arriveStationIds"]').filter(':checked').each(function () {
            var array = [{lat: markerGreen.getPosition().lat(), lng: markerGreen.getPosition().lng()},{lat: $(this).data('lat'), lng: $(this).data('lng')}];
    ...
    function clearLines() {
        for(var i=0; i<neighbourStations.length; i++){
            console.log('neighbourStations', neighbourStations);
